...
class ConfigurationManager(metaclass=Singleton):
    CONFIGURATION_PATH: Final[str] = '/etc/ciy-worker-manager/config.json' if sys.platform == 'linux ' else \
        f"{os.environ['USERPROFILE']}\\.ciy-worker-manager\\config.json"
